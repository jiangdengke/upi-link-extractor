...
account. Logic giống probe nhưng:
    ...
    - Hardcoded các knob theo yêu cầu UI:
        ...
        confirm_variants=("payment_method",)
...
# UPI ưu tiên chrome142 để giảm CF 403 khi đổi proxy. Chỉ giữ các target được
# curl_cffi 0.14+ hỗ trợ; chrome145 từng làm TLS recovery chết ngay bằng
# ImpersonateError trước khi request có thể retry.
_IMPERSONATE_CHAIN: tuple[str, ...] = ("chrome142", "chrome136")
...
#
# Pattern đồng bộ với request_phase._is_tls_error.
...
def _materialize_or_log_warning(line: str | None, log: "LogFn") -> str | None:
    ...
    try:
        ...
        log(_fmt_step(
            ...
            f"line {masked!r} parse fail: {exc} → SILENT DIRECT (FIX line in "
...
# Số approve request chạy SONG SONG mỗi "wave" ở Step 6. 1 = tuần tự (mặc
# định, giữ nguyên hành vi cũ). >1 = mỗi wave bắn N request đồng thời, mỗi
# request 1 session curl_cffi ĐỘC LẬP (isolate BoringSSL/cookie state) + 1 IP
# proxy khác nhau (spread across pool, per_proxy_attempts=1). Kết quả mỗi wave
# vẫn được classify TUẦN TỰ theo đúng semantics cũ (counter/restart/block-streak
...
CONFIRM_VARIANTS: tuple[str, ...] = ("payment_method",)
...
def _extract_qr_artifacts(
    sources: list[tuple[str, Any]],
) -> dict[str, Any]:
    """Extract usable UPI QR artifacts from Stripe/OpenAI responses."""
    matches: list[dict[str, Any]] = []
    for source, value in sources:
        if value is not None:
            matches.extend(_find_matches(value, source=source))
    return {
...
def _has_usable_qr_artifact(artifacts: dict[str, Any]) -> bool:
    return any(
        artifacts.get(key)
        for key in ("upi_uri", "qr_image_url", "payment_link")
    )
...
def _stripe_return_url(session_id: str) -> str:
    return (
        f"https://pay.openai.com/c/pay/{session_id}"
...
def _mask_email(email: str | None) -> str:
    """Mask account email before embedding it in a QR artifact."""
    if not email or "@" not in email:
        return "***@****"
    local, _, domain = email.partition("@")
    masked_local = "***" if len(local) <= 3 else local[:-3] + "***"
    dot = domain.rfind(".")
    masked_domain = "****" if dot <= 0 or dot == len(domain) - 1 else "****" + domain[dot:]
...
def _overlay_email_on_qr(qr_path: Path, email: str) -> None:
    ...
    Email mask được ghi trực tiếp vào ảnh QR để không lộ tài khoản đầy đủ.
...
def _overlay_email_on_svg(qr_path: Path, masked: str) -> None:
    ...
    if root.tag.split("}")[-1] != "svg":
        raise ValueError(f"root element is not <svg> (tag={root.tag})")
    ...
    parts = vb_raw.split()
    if len(parts) != 4:
        raise ValueError(f"invalid viewBox: {vb_raw!r}")
...
# 日志给操作员看的阶段名统一用中文；协议字段（Checkout、Stripe、QR）
# 保留英文缩写，方便和上游响应及排障资料对应。
_LOG_LABELS: dict[str, str] = {
    "upi": "UPI",
    "account": "账号",
    "config": "配置",
    "proxy policy": "代理策略",
    "login proxy override": "登录代理覆盖",
    "login": "登录",
    "checkout": "Checkout会话",
    "init": "Stripe初始化",
    "stripe_init": "Stripe初始化",
    "elements": "Stripe元素",
    "token-config": "令牌配置",
    "confirm": "确认支付",
    "payment-method": "支付方式",
...
_LOG_DETAIL_TRANSLATIONS: tuple[tuple[str, str], ...] = (
    ("using provided access token; login skipped", "使用提供的访问令牌，跳过登录"),
    ("login skipped", "跳过登录"),
    ("user is already paid", "账号已经付费"),
    ("confirm failed for every variant", "所有确认方式均失败"),
    ("approve did not succeed after", "授权尝试次数耗尽，仍未成功（"),
    ("no upi:// URI or QR image URL found in any response", "响应中未找到 UPI 地址或二维码图片链接"),
    ("no QR generated", "未生成二维码"),
    ("account already paid; QR is not applicable", "账号已经付费，不需要新的二维码"),
    ("network error", "网络错误"),
    ("removed from pool", "已从代理池移除"),
    ("all entries were marked dead", "所有代理都被标记为不可用"),
    ("flow will run DIRECT", "流程将直连执行"),
    (" attempts ", " 次尝试 "),
    ("retries=", "重试次数="),
    ("restarts=", "重启次数="),
...
def _localize_log_detail(detail: str) -> str:
    """Translate stable operator-facing text while preserving diagnostics."""
    localized = detail
    for source, target in _LOG_DETAIL_TRANSLATIONS:
...
def _fmt_step(step: str, label: str, status: str = "info", detail: str = "") -> str:
    ...
    label_token = _LOG_LABELS.get(label, label).ljust(_LABEL_WIDTH)
    line = f"{step_token}{label_token}{icon}"
    if detail:
        line += f"  {_localize_log_detail(detail)}"
    return line
...
def _fmt_attempt(
    *, idx: int, total: int,
    ...
    r = _ATTEMPT_RESULTS.get(raw_result, raw_result).ljust(10)
    base = f"      尝试 {n:<8}  {icon}  {h:<9}  {r}  代理={proxy_mask}"
...
class _RotatingSession:
    ...
    def __init__(self, impersonate_chain: tuple[str, ...], *, log: LogFn):
        if not impersonate_chain:
            raise ValueError("impersonate_chain must not be empty")
    ...
    def __getattr__(self, name: str):
        ...
        if inner is None:
            raise AttributeError(f"_RotatingSession has not entered context: .{name}")
...
async def _update_chatgpt_checkout(
    sess: Any,
    *,
    access_token: str,
    session_id: str,
...
async def _create_chatgpt_checkout_with_proxy_split(
    ...
    *,
    ...
    Since login is done outside the engine session (injected ``login_fn``), the
    session's cookie jar has no Cloudflare ``__cf_bm`` for chatgpt.com. Warm it
    on the same proxy right before hitting ``/checkout`` (and again on the promo
    proxy before ``/checkout/update``) to avoid the first-packet CF 403 HTML
...
async def _stripe_create_upi_payment_method(
    sess: Any,
    *,
    session_id: str,
    publishable_key: str,
    ...
    """Create the UPI PaymentMethod required by payment_pages/confirm."""
    from .pay_upi_http import (
        _STRIPE_PM_URL, _STRIPE_VERSION, _USER_AGENT,
    ...
    client_attribution_metadata = {
        "checkout_config_id": init_data.get("config_id") or "",
        "checkout_session_id": session_id,
        "client_session_id": stripe_js_id,
        "elements_session_config_id": elements_data.get("config_id") or "",
        "elements_session_id": elements_data.get("session_id") or "",
    ...
    form = _to_form({
        ...
        "guid": stripe_client_ids["guid"],
        "muid": stripe_client_ids["muid"],
        "sid": stripe_client_ids["sid"],
        "_stripe_version": _STRIPE_VERSION,
        "key": publishable_key,
    })
    headers = {
    ...
    payment_method_id = data.get("id") if isinstance(data, dict) else None
    error = (
        _redact_error(data.get("error"))
        if isinstance(data, dict) and data.get("error")
        else None
    )
    return {
        "http_status": resp.status_code,
        "ok": resp.status_code == 200 and isinstance(payment_method_id, str),
        "payment_method_id": payment_method_id,
        "stripe_client_ids": stripe_client_ids,
        "error": error,
    }
...
async def _stripe_confirm_upi_qr(
    ...
    *,
    ...
    stripe_client_ids: dict[str, str],
...
async def _chatgpt_approve_checkout(
    ...
    *,
    ...
    return {
        ...
        "error": (
            _redact_error(data.get("error"))
            if isinstance(data, dict) and data.get("error") else None
        ),
        "data": data if resp.status_code == 200 else None,
    }
...
async def _warm_cf_cookie(
    ...
    *,
    ...
    try:
        ...
        log(_fmt_step(
            step, "cf-warmup", "warn",
            f"skip ({type(exc).__name__}: {str(exc)[:80]}) — approve will rotate on 403",
        ))
        return
...
async def _approve_once_isolated(
    *,
    ...
    """1 approve attempt trên session curl_cffi ĐỘC LẬP — dùng cho concurrent
    wave (``approve_concurrency > 1``).
    ...
    network/TLS fail được gói thành attempt ``http_status=None`` để classifier
    xử lý y hệt path tuần tự.
    """
    materialized = _safe_materialize(raw_proxy)
    proxies = _proxy_dict(materialized)
    try:
    ...
    attempt["_materialized_proxy"] = materialized
    return attempt
...
def _contains_text(value: Any, needle: str) -> bool:
    """Case-insensitive recursive text search trong response dict/list."""
...
def _is_user_already_paid(attempt: dict[str, Any]) -> bool:
    """Stripe/OpenAI báo account đã paid rồi.

    Đây là trạng thái thành công thực tế: account đã lên Plus/paid nên không
    restart checkout session nữa, chỉ để check_plan xác nhận badge PLUS.
    """
    return _contains_text(attempt, "user is already paid")
...
async def _wait_network_recovery(sess: Any, log: LogFn) -> bool:
    ...
    while True:
        ...
        if elapsed > NETWORK_RECOVERY_MAX_WAIT_SECONDS:
            log(_fmt_step(
                "net", "outage", "fail",
                f"did not recover within {NETWORK_RECOVERY_MAX_WAIT_SECONDS:.0f}s",
            ))
            return False
        ...
        if ok:
            log(_fmt_step(
                "net", "recovered", "ok",
                f"after {elapsed:.0f}s ({poll_idx} probes) — resume approve loop",
            ))
            return True
...
async def run_upi_qr_probe(
    *,
    ...
    approve_concurrency: int = APPROVE_CONCURRENCY,
    ...
    if approve_retries < 1:
        raise UpiQrError(f"approve_retries must be >= 1, got {approve_retries}")
    if restart_threshold < 0:
        raise UpiQrError(f"restart_threshold must be >= 0, got {restart_threshold}")
    if max_restarts < 0:
        raise UpiQrError(f"max_restarts must be >= 0, got {max_restarts}")
    if relogin_block_streak < 0:
        raise UpiQrError(f"relogin_block_streak must be >= 0, got {relogin_block_streak}")
    if not (1 <= proxy_from_step <= 6):
        raise UpiQrError(
            f"proxy_from_step must be in [1, 6], got {proxy_from_step}"
        )
    if not (1 <= approve_concurrency <= 20):
        raise UpiQrError(
            f"approve_concurrency must be in [1, 20], got {approve_concurrency}"
        )
    # Resolve approve delay — None → fallback module default (`APPROVE_DELAY`).
    # Below 5s will hit Stripe's approve rate-limit (`blocked` http=200 repeated)
    # → validate at the boundary to fail-fast if the caller passes too small a value.
    if approve_retry_delay is None:
        ...
        if not (2.0 <= effective_approve_delay <= 60.0):
            raise UpiQrError(
                f"approve_retry_delay must be in [2, 60] seconds, got {approve_retry_delay}"
            )
    ...
    if proxy_pool and first_proxy is None:
        raise UpiQrError(
            f"proxy_pool has {len(proxy_pool)} entries but first_proxy "
            f"resolved to None — proxy[0] format is invalid. See the "
            f"`[proxy] format-error warn` log above for details. "
            f"Fix the format in Settings tab > Proxy Pool: `host:port[:user[:pass]]`."
        )
    _step_names = {
    ...
    for s in range(1, 7):
        label = _step_names[s]
        if first_proxy and (s == 2 or s >= proxy_from_step):
    ...
    if _login_proxy_raw:
        _safe_log(_fmt_step(
            ...
            f"(fallback DIRECT + Step 2 follows proxy_from_step if the proxy "
            f"is dead or malformed)",
        ))
    ...
    def _step_proxy_tag(step: int) -> str:
        ...
        """
        if first_proxy and (step == 2 or step >= proxy_from_step):
    ...
    for login_attempt in range(1, LOGIN_MAX_ATTEMPTS + 1):
        ...
        if requested_login_proxy and login_attempt <= LOGIN_PROXY_RETRY_QUOTA:
            ...
            if requested_login_proxy and not proxy_fallback_logged:
                _safe_log(_fmt_step(
                    "1/6", "login", "warn",
                    f"login proxy failed {LOGIN_PROXY_RETRY_QUOTA} times → fallback DIRECT "
        ...
        try:
            ...
            if login_attempt > 1:
                via = "via proxy" if current_proxy else "DIRECT"
                _safe_log(
                    f"[upi-qr] login OK on attempt {login_attempt}/{LOGIN_MAX_ATTEMPTS} ({via})"
                )
            break
    ...
    if not isinstance(access_token, str) or not access_token:
        _safe_log(_fmt_step("1/6", "login", "fail", "no accessToken in response"))
        return UpiQrResult(
            ok=False, email=masked_email,
            error="login OK but no accessToken",
            elapsed_seconds=monotonic() - started,
            proxy_used=first_proxy,
        )
    ...
    async with _RotatingSession(_IMPERSONATE_CHAIN, log=_safe_log) as sess:
        while True:
            ...
            try:
                checkout = await _create_chatgpt_checkout_with_proxy_split(
                    sess,
                    access_token=access_token,
                ...
                if _is_user_already_paid({"error": str(exc)}):
                    already_paid_detected = True
                    approved = True
                    final_confirmed = True
                    final_approved = True
                    _safe_log(_fmt_step(
                        f"2/6{phase_tag}", "checkout", "ok",
                        "user is already paid -> check_plan sẽ xác nhận Plus",
                    ))
                    break
            ...
            _safe_log(_fmt_step(
                ...
                f"{_step_proxy_tag(2)}  "
                f"promo_proxy={_mask_proxy(effective_login_proxy) if effective_login_proxy else 'DIRECT'}",
            ))
            ...
            _safe_log(_fmt_step(
                ...
                f"pm={','.join(str(item) for item in payment_method_types) or '-'}  "
                f"{_step_proxy_tag(3)}",
            ))
            if PROMO and amount > 0:
                _safe_log(_fmt_step("upi", "no free offer", "fail",
                                    f"amount={amount} (promo enabled but > 0)"))
                if restart_count == 0:
                    return UpiQrResult(
            ...
            # Step 5b — Stripe now requires an explicit pm_... identifier.
            payment_method = await _stripe_create_upi_payment_method(
                sess,
                session_id=session_id,
                publishable_key=publishable_key,
                stripe_js_id=stripe_js_id,
            ...
            pm_ok = bool(payment_method.get("ok"))
            pm_detail = (
                f"http={payment_method.get('http_status')}  "
                f"{_step_proxy_tag(5)}"
            )
            pm_error = payment_method.get("error")
            if isinstance(pm_error, dict):
                pm_code = pm_error.get("code") or pm_error.get("type") or ""
                if pm_code:
                    pm_detail += f"  err={pm_code}"
            _safe_log(_fmt_step(
                f"5b{phase_tag}", "payment-method", "ok" if pm_ok else "fail", pm_detail,
            ))
            ...
            if not pm_ok:
                confirm_attempts.append({
                    "variant": "payment_method",
                    "phase": phase_idx,
                    "http_status": payment_method.get("http_status"),
                    "ok": False,
                    "keys": [],
                    "error": pm_error,
                })
                ...
                for variant in CONFIRM_VARIANTS:
                    attempt = await _stripe_confirm_upi_qr(
                        ...
                        stripe_client_ids=payment_method["stripe_client_ids"],
                    ...
                    _safe_log(_fmt_step(
                        f"5b{phase_tag}", "confirm", confirm_status, confirm_detail,
                    ))
                    if attempt.get("ok"):
                        phase_confirmed = True
                        confirm_variant_used = variant
                        break
                    if _is_user_already_paid(attempt):
                        already_paid_detected = True
                        approved = True
                        phase_confirmed = True
                        final_confirmed = True
            ...
            if not phase_confirmed:
                if restart_count == 0:
                    # Preserve the full sanitized Stripe failure in job details.
                    break
                fatal_approve_error = (
                    f"phase {phase_idx} confirm failed for every variant"
                )
                _safe_log(_fmt_step(
            ...
            qr_sources: list[tuple[str, Any]] = [
                ("chatgpt_checkout", checkout),
                ("stripe_init", init_data),
                ("stripe_elements", elements_data),
            ]
            qr_sources.extend(
                (f"confirm:{attempt['variant']}", attempt.get("data"))
                for attempt in confirm_attempts
            )
            qr_sources.extend(
                (f"payment_page_refresh:{index}", attempt.get("data"))
                for index, attempt in enumerate(page_refresh_attempts, start=1)
            )
            early_qr_artifacts = _extract_qr_artifacts(qr_sources)
            if _has_usable_qr_artifact(early_qr_artifacts):
                _safe_log(_fmt_step(
                    "qr", "response", "ok",
                    "UPI artifact found after confirm; approve loop skipped",
                ))
                break
            ...
            async def _register_and_classify(
                ...
                *,
                approve_proxy: str | None,
                attempt_index: int,
            ) -> str:
                """Đăng ký 1 approve attempt vào history + log + phân loại →
                trả control-signal cho caller:

                  ``"stop"``      — break approve loop (approved / already-paid /
                                    relogin block-streak / restart / fatal /
                                    network-outage-fatal).
                  ``"retry-now"`` — tiếp tục NGAY, KHÔNG sleep (network vừa
                                    recover; không đốt thêm delay budget).
                  ``"retry"``     — attempt fail bình thường; caller sleep rồi
                                    tiếp attempt/wave kế.

                Mutate nonlocal counters/flags y hệt logic tuần tự cũ. Gọi TUẦN
                TỰ (kể cả cho các attempt của 1 concurrent wave) nên semantics
                counter/restart/block-streak KHÔNG đổi so với bản sequential.
                """
                nonlocal approved, already_paid_detected, relogin_requested
                nonlocal triggered_restart, fatal_approve_error
                nonlocal backend_exception_count, consecutive_backend_exception
                nonlocal consecutive_network_error, consecutive_reject
                nonlocal proxy_virtual_attempt
                approve_attempt["variant"] = confirm_variant_used
                approve_attempt["attempt"] = attempt_index
                ...
                _safe_log(_fmt_attempt(
                    idx=attempt_index, total=approve_retries,
                ...
                if approve_attempt.get("ok"):
                    approved = True
                    return "stop"
                if _is_user_already_paid(approve_attempt):
                    already_paid_detected = True
                    approved = True
                    _safe_log(_fmt_step(
                        "6/6", "approve", "ok",
                        "user is already paid → coi như Plus, không restart",
                    ))
                    return "stop"
                    ...
                    if _is_retryable_reject(approve_attempt):
                        ...
                        if consecutive_reject >= relogin_block_streak:
                            ...
                            _safe_log(_fmt_step(
                                ...
                                f"consecutive IP/edge-rejects → breaking cycle to re-login",
                            ))
                            return "stop"
                    else:
                ...
                if _is_network_error(approve_attempt):
                    ...
                    if consecutive_network_error >= NETWORK_FAIL_DETECT:
                        _safe_log(_fmt_step(
                            "net", "outage", "warn",
                            f"{consecutive_network_error} consecutive timeouts/network "
                            f"errors → pause approve loop, poll connectivity",
                        ))
                        ...
                        if ok_recover:
                            ...
                            return "retry-now"
                        fatal_approve_error = (
                            f"network outage did not recover within "
            ...
            if approve_concurrency <= 1:
                # ── Tuần tự (mặc định) — session dùng chung + sticky batch/warm.
                while approve_index_total < approve_retries:
                    if should_cancel and should_cancel():
                        _safe_log("[cancel] 收到取消请求 → 中断 approve 循环")
                        break
                    ...
                    signal = await _register_and_classify(
                        approve_attempt,
                        approve_proxy=approve_proxy,
                        attempt_index=approve_index_total,
                    )
                    if signal == "stop":
                        break
                    if signal == "retry-now":
                ...
                # ── Concurrent wave (approve_concurrency > 1) — mỗi slot 1
                # session curl_cffi ĐỘC LẬP + 1 IP proxy kế tiếp (spread across
                # pool, per_proxy_attempts=1). Bắn song song rồi classify TUẦN
                # TỰ theo thứ tự slot để giữ nguyên semantics counter/restart/
                # block-streak. Approved bất kỳ slot nào → dừng ngay.
                while approve_index_total < approve_retries:
                    if should_cancel and should_cancel():
                        _safe_log("[cancel] 收到取消请求 → 中断 approve 循环")
                        break
                    wave = min(
                        approve_concurrency, approve_retries - approve_index_total
                    )
                    slots: list[tuple[int, str | None]] = []
                    for _ in range(wave):
                        approve_index_total += 1
                        proxy_virtual_attempt += 1
                        raw = _proxy_url_for_retry(
                            proxy_pool,
                            from_step=proxy_from_step,
                            step=6,
                            attempt=proxy_virtual_attempt,
                            per_proxy_attempts=1,  # spread: mỗi slot IP kế tiếp
                        )
                        slots.append((approve_index_total, raw))
                    ...
                    stop = False
                    retry_now = False
                    for (idx, _raw), approve_attempt in zip(slots, results):
                        approve_proxy = approve_attempt.pop("_materialized_proxy", None)
                        signal = await _register_and_classify(
                            approve_attempt,
                            approve_proxy=approve_proxy,
                            attempt_index=idx,
            ...
            if approved:
                ...
                _safe_log(_fmt_step(
                    "6/6", "approve", "ok",
                    f"{'already paid' if already_paid_detected else 'approved'} "
                    f"at {approve_index_total}/{approve_retries}  "
            ...
            if approve_index_total >= approve_retries:
                ...
                _safe_log(_fmt_step(
                    "6/6", "approve", "fail",
                    f"not approved after {approve_retries} attempts "
                    f"({approve_elapsed:.1f}s, restarts={restart_count})",
                ))
                break
        ...
        for attempt in confirm_attempts:
            qr_sources.append((f"confirm:{attempt['variant']}", attempt.get("data")))
        for attempt in approve_attempts:
            qr_sources.append((f"approve:{attempt['variant']}", attempt.get("data")))
        for index, attempt in enumerate(page_refresh_attempts, start=1):
            qr_sources.append((f"payment_page_refresh:{index}", attempt.get("data")))
        qr_artifacts = _extract_qr_artifacts(qr_sources)
        upi_uri = qr_artifacts["upi_uri"]
        qr_image_url = qr_artifacts["qr_image_url"]
        qr_expires_at = qr_artifacts["qr_expires_at"]
        payment_link = qr_artifacts["payment_link"]

        # QR rendering (download Stripe image hoặc render từ upi:// URI).
        qr_path: str | None = None
        qr_source: str | None = None
        qr_reason: str | None = None
        qr_download_url = qr_image_url or payment_link
        if qr_download_url:
            extension = ".svg" if qr_download_url.lower().endswith(".svg") else ".png"
            target = qr_out_path.with_suffix(extension)
            qr_dl = await _download_qr_image(
                sess, url=qr_download_url, out_path=target,
                proxies=_proxy_for_step(first_proxy, from_step=proxy_from_step, step=5),
            )
        ...
        elif already_paid_detected:
            qr_reason = "account already paid; QR is not applicable"
        else:
            qr_reason = "no upi:// URI or QR image URL found in any response"

        # Overlay email masked lên ảnh QR để nhận diện artifact mà không lộ email.
        # Best-effort: lỗi overlay log riêng, KHÔNG bỏ qr_path — QR raw vẫn dùng
        # được. Path canonical ghi đè in-place, nên web modal + clipboard tự
        # động dùng ảnh đã có watermark.
        if qr_path:
            try:
        ...
        if qr_path:
            ...
            _safe_log(_fmt_step(
                "qr", "saved", "skip",
                qr_reason or "account already paid; QR is not applicable",
            ))
        else:
    ...
    # Determine success: bình thường cần CẢ approve approved + qr file rendered.
    # Riêng "user is already paid" nghĩa là account đã paid/Plus rồi, không cần
    # QR mới; check_plan sẽ xác nhận badge PLUS ngay sau đó.
    ...
    if fatal_approve_error:
        ...
        error_msg = "confirm failed for every variant"
    ...
    return UpiQrResult(
        ...
        already_paid=already_paid_detected,
...
__all__ = [
    ...
    "APPROVE_DELAY", "APPROVE_PROXY_BATCH", "APPROVE_CONCURRENCY",
    "APPROVE_BACKEND_EXCEPTION_CONSECUTIVE",
